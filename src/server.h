...
private:
  ...
  std::atomic<int>
  currJobSFD_;     //!< Socket file descriptor for active client (-1 while no active job)
  ...
  std::atomic<bool>
  acceptJob_;     //!< Flag to indicate Server can set currInput_
  std::atomic<bool>
  jobCompleted_;  //!< Flag to indicate we can send currOutput_ to activeClient_
