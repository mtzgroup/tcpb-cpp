...
using TCPB::Output;
#include "socket.h"
using TCPB::Socket; using TCPB::SelectServerSocket;
...
Server::Server(int port) {
  socket_ = new SelectServerSocket(port, &Server::);
...
bool Server::SendJobAsync(const Input& input) {
  uint32_t header[2];
  bool sendSuccess, recvSuccess;
  int msgType, msgSize;
  string msgStr;
...
const Output Server::RecvJobAsync() {
  uint32_t header[2];
  bool recvSuccess;
  int msgType, msgSize;
  string msgStr;
...
const Output Server::ComputeJobSync(const Input& input) {
  // Try to submit job
...
const Output Server::ComputeEnergy(const Input& input,
                                           double& energy) {
...
const Output Server::ComputeGradient(const Input& input,
                                             double& energy,
                                             double* gradient) {
...
const Output Server::ComputeForces(const Input& input,
                                           double& energy,
                                           double* gradient) {
